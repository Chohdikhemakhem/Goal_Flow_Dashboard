...
def parse_date(value) -> Optional[date]:
    """
    Parse une date reçue du frontend sous forme de string.
    Accepte les formats suivants :
      - ISO : "2026-09-10"
      - Français : "10/09/2026"
      - None / vide : retourne None
    """
    if not value:
        return None
...
def validate_and_map_excel_data(parsed_data: dict, db: Session) -> dict:
    ...
    if month is None or year is None:
        return {
            "success": False,
            "error": "Mois/annee non detectes. Veuillez les selectionner manuellement.",
            "month": None,
            "year": None,
            "rows": [],
        }
...
async def preview_objectives_import(
    ...
    return {
        ...
        "title": parsed["title"],
        "rows": result["rows"],
        "detected_agencies": result["detected_agencies"],
        "valid_agencies": result["valid_agencies"],
        "errors": result.get("errors", []),
        ...
        for row in rows:
            ...
            if existing:
                ...
                if action == "update":
                    ...
                    existing.active_from = parse_date(row.get("active_from"))
                    existing.active_until = parse_date(row.get("active_until"))
                    updated += 1
                continue
